/*******************************************************************
   ...
   In Dynamic C Options/Project Options ~ Board Selection tab, select:

	   58Mhz, RCM4200, 512K+512K SRAM, 512K Flash, analog, 8M serial flash
 ...
 Inspection Control Signal from PLC
 	...
 	low voltage signal from PLC (read as 1 in Rabbit) -- no pipe in system
 	low to high voltage transition from PLC (read as 1 to 0) --
	   leading vertical eye has detected end of tube
   short pulse high-low-high from PLC(read as 0-1-0) --head 1 lowered or raised
   longer pulse high-low-high from PLC(read as 0-1-0) --head 2 lowered or raised
   longest pulse high-low-high from PLC(read as 0-1-0)--head 3 lowered or raised
	high to low for longer time than above from PLC (read as 0-1) --
...
//head1 up/down signal is 100 mS
//head2 up/down signal is 200 ms for three-head units and 300 for two-head
//head3 up/down signal is 300 ms
//end of pipe signal is 400 ms or longer
//each count of the width values below represents .01 seconds

// signal widths for the heads is out of order to maintain compatibility
//with older systems with only two heads -- can move these values to the
//Main Static Settings.ini file to customize for each unit -- if no entry
//found in that file, then head 2 = 30, otherwise head 2 = 20 and head = 30

#define HEAD1_SIGNAL_WIDTH 10	//width of signal pulse for head 1 up/down
#define HEAD2_SIGNAL_WIDTH 20	//width of signal pulse for head 2 up/down
#define HEAD3_SIGNAL_WIDTH 30	//width of signal pulse for head 3 up/down
#define OFF_PIPE_SIGNAL_WIDTH 40 //width of signal pulse for off pipe (or longer)

#define SIGNAL_WIDTH_WINDOW 4 //+/- allowance for the above signal widths

#define INSPECT_CONTROL_DEGLITCH_COUNT 20

//FREQUENCY CRITICAL: these are based on the TimerB frequency of 14160 Hz
//							 scale them appropriately if frequency is changed
#define TDC_DEGLITCH_COUNT 40
#define TRACK_SYNC_PULSE_WIDTH 80
#define TRACK_RESET_PULSE_WIDTH 80
#define TRACK_RESET_PULSE_HALF_WIDTH 40
//FREQUENCY CRITICAL END
...
int sendInspectPacket(tcp_Socket *socket)
{
   ...
   if (head3Down) controlFlags = (controlFlags | HEAD3_DOWN_CTRL);
...
void parseInspectionCtrl(int pInspectionCtrlNow)
{
   ...
   //if the signal went low while already on pipe (off pipe caught above), and
   ...
   //head 1 was raised or lowered and a high pulse was received from PLC
	if((pInspectionCtrlNow == 0)
   		&& (inspectionCtrlTime > (HEAD1_SIGNAL_WIDTH - SIGNAL_WIDTH_WINDOW))
         && (inspectionCtrlTime < (HEAD1_SIGNAL_WIDTH + SIGNAL_WIDTH_WINDOW))){
   ...
   //head 2 was raised or lowered and a high pulse was received from PLC
	if((pInspectionCtrlNow == 0)
   		&& (inspectionCtrlTime > (HEAD2_SIGNAL_WIDTH - SIGNAL_WIDTH_WINDOW))
         && (inspectionCtrlTime < (HEAD2_SIGNAL_WIDTH + SIGNAL_WIDTH_WINDOW))){
   ...
   //if the signal went low while already on pipe (off pipe caught above), and
   //time duration of previous state is near HEAD3_SIGNAL_WIDTH, then
   //head 3 was raised or lowered and a high pulse was received from PLC
	if((pInspectionCtrlNow == 0)
   		&& (inspectionCtrlTime > (HEAD3_SIGNAL_WIDTH - SIGNAL_WIDTH_WINDOW))
         && (inspectionCtrlTime < (HEAD3_SIGNAL_WIDTH + SIGNAL_WIDTH_WINDOW))){

		//switch state of head 3
		if (head3Down == FALSE){
      	head3Down = TRUE;
         inspectionStatus=7;
			encoderPosAtHead3DownSignal = enc2CntTemp.lVal;
      } else {
      	head3Down = FALSE;
			inspectionStatus=8;
			encoderPosAtHead3UpSignal = enc2CntTemp.lVal;
         }
      return;
	}
...
//
// The encoder position stored is the value trapped for encoder2 when inspection
// control signal changed state. This function is not called until .4 seconds
// after the end-of-pipe change, so the trapped value is used to maintain
// accuracy as the encoders would have been turning for that time after the
// actual end of the pipe.
//
...
int setToOffPipeState()
{
   ...
   //store the linear encoder position trapped at signal change
   encoderPosAtOffPipeSignal = encoderPosAtLastSigChange;
      ...
      //(input bit set to 0 when eye receives reflection)
    	...
    	if (!tdcDeglitched){
         ...
         prevTDCPeriodCount = tdcPeriodCount;
      	...
      	//calculate RPM
         if (tdcPeriodCount > 0)
      		rpm = (int)((1.0 / (tdcPeriodCount * TIMERB_PERIOD)) * 60.0);
			else
         	rpm = 0;
...
// every .01 seconds. Handles all events driven by the timer.
//
// Handles turning off output pulses when they have timed out.
//
// Note that countBTemp is never reset, but at timer B rate of 14.16Khz, it will
// not roll over for many hours.
//

void handleTimerBEvents()
{
   ...
   handleTrackSyncAndResetLines(countBTemp);
	...
	if (countBTemp - prevCountBTemp >= TIMERB_SCALER_10MS){
   ...
   //it was in the last state and start timing the new state
	if(inspectionCtrl != inspectionCtrlNow){
   	...
   	encoderPosAtLastSigChange = enc2CntTemp.lVal;
...
int startMonitor(tcp_Socket *socket, int pPktID)
{
   ...
   //debug mks -- should return the number of bytes?
...
dec2:	; decrement the encoder count
	...
	//no need to reload the match 1 registers as TimerB is configured to
   //reload those from the step 1 registers
...
// PCLK/16.
//
// The timer step registers are set to 256, which results in the counter
// triggering at each 256 counts.  The resulting interrupt frequency:
//
// PCLK = 58 Mhz
// Interrupt frequency = 58Mhz / 16 / 256 = 14160 Hz = 14.16 Khz
//
// As the step registers are enabled, the timer match will reload automatically
// and the ISR will not have to perform this task.
//
...
void setupTimerBInt()
{
   ...
   //  auto reload match registers from step registers
   WrPortI(TBCR, &TBCRShadow,
   				TBCR_CLOCK_PCLK16 | TBCR_INT_PRI_2 | TBCR_USE_STEP1_REGS);

   //load the step registers which will be used to automatically reload
   //the match registers

   WrPortI(TBSM1R, NULL, TIMERB_STEP1_MSB);
   WrPortI(TBSL1R, NULL, TIMERB_STEP1_LSB);

   // set initial match - use zero for 1024 counts for simplicity
   // first interrupt will reload with proper value
